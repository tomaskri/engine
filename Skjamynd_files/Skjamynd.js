...
window.onload = function init() {
    ...
    var proj = perspective( 15.0, 1.0, 0.2, 100.0 );
...
var render = function(){
   ...
   /*  mv = mult( mv, translate(-0.5, 0.0, 0.0) );
    ...
    mv1 = mult( mv1, scalem( 0.7, 0.5, 0.03 ) );
    gl.uniformMatrix4fv(locModelView1, false, flatten(mv1));
    gl.drawArrays( gl.TRIANGLES, 0, numPoleVertices );

    // Fótur
    mv1 = mult( mv, translate( 0.0, -0.3, -0.04 ) );
    mv1 = mult( mv1, scalem( 0.1, 0.3, 0.05 ) );
    gl.uniformMatrix4fv(locModelView1, false, flatten(mv1));
    gl.drawArrays( gl.TRIANGLES, 0, numPoleVertices );

    // Standur
    mv1 = mult( mv, translate( 0.0, -0.45, -0.02 ) );
    mv1 = mult( mv1, scalem( 0.5, 0.04, 0.12 ) );
    gl.uniformMatrix4fv(locModelView1, false, flatten(mv1));
    gl.drawArrays( gl.TRIANGLES, 0, numPoleVertices );
    ...
    mv2 = mult( mv2, translate(0.0, 0.0, 0.0155) );
    mv2 = mult( mv2, scalem(0.23, 0.2, 0.2) );
